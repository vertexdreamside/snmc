...
export function BallotForm({
  ...
  // Categories the person already voted in (per the page's
  // vote_participation check) start as "submitted" so a re-visit to this
  // page shows the recorded-vote message instead of the ballot again —
  // this is the display-side half of Section 20 (can't be shown a ballot
  // you can no longer actually submit).
      ...
      {sections.map(({ category, candidates }) => (
        <section key={category} className="bg-white rounded-card border border-council-navy/10 p-6">
          ...
          {submitted[category] ? (
            ...
            <p className="font-body text-sm text-council-ink/50">No accepted candidates yet in this category.</p>
          ) : (
