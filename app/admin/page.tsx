...
export default async function AdminDashboard() {
  ...
  const showRegister = canManageRegister(admin.role) || isReportingOnly(admin.role);
  const showElections = canManageElections(admin.role) || isReportingOnly(admin.role);
  ...
  const [{ count: totalPeople }, { count: pendingReview }, { count: openElections }, { count: unconfirmedCategory }, statusCounts] =
    await Promise.all([
      ...
      showElections
        ? supabase.from("elections").select("*", { count: "exact", head: true }).in("status", ["Round 1 Open", "Round 2 Open"])
        : Promise.resolve({ count: 0 }),
      showRegister
    ...
    <div className="space-y-6 max-w-6xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="font-body text-sm text-council-ink/50">
            {admin.role} · here's what's relevant to your role today.
          </p>
        </div>
      </div>

      {showRegister && (unconfirmedCategory ?? 0) > 0 && canManageRegister(admin.role) && (
        <a
          href="/admin/register/classify"
          className="block bg-white rounded-card border-2 border-council-gold p-4 hover:bg-council-cream transition-colors"
        >
      ...
      {(showRegister || showElections) && (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {showRegister && (
            <>
              <StatCard icon={Users} iconBg="#1E7D4F" value={totalPeople ?? 0} label="Total Registered" />
              <StatCard icon={Heart} iconBg="#378ADD" value={nurseCount} label="Currently Practising" />
              <StatCard icon={ClipboardCheck} iconBg="#C9922B" value={pendingReview ?? 0} label="Pending Approval" />
              <StatCard icon={ShieldQuestion} iconBg="#C9922B" value={unconfirmedCategory ?? 0} label="Category Not Confirmed" />
              <StatCard icon={AlertTriangle} iconBg="#D4534B" value={unknownCount} label="Unknown Status — Needs Review" />
              <StatCard
                icon={FileWarning}
                iconBg="#8A2C2C"
          ...
          {showElections && <StatCard icon={Vote} iconBg="#7F77DD" value={openElections ?? 0} label="Elections Currently Open" />}
        </div>
      )}
      ...
      {!showRegister && !showElections && (
        <div className="bg-white rounded-card border border-council-navy/10 p-6">
          <p className="font-body text-sm text-council-ink/50">
            Your role ({admin.role}) doesn't have dashboard sections configured yet. Visit Reports for available data.
          </p>
        </div>
      )}
    </div>
...
function StatCard({
  ...
  return (
    <div className="bg-white rounded-card border border-council-navy/10 p-5">
      <div
        className="w-10 h-10 rounded-card flex items-center justify-center mb-4"
