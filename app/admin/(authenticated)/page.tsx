...
export default async function AdminDashboard() {
  ...
  // headline totals (they still exist in the full register/database for
  // historical record-keeping, this only affects what these two
  // dashboard numbers count).
  const nurseCount = register.filter((p: any) => (p.professional_category === "Nurse" || p.professional_category === "Both") && !p.is_deceased).length;
  const midwifeCount = register.filter((p: any) => (p.professional_category === "Midwife" || p.professional_category === "Both") && !p.is_deceased).length;
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <NotificationsPanel notifications={notifications} readKeys={readKeys} />
