...
export default async function ElectionDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: election } = await supabase
    .from("elections")
    .select("id, term_label, status, results_published")
    .eq("id", params.id)
    .single();

  if (!election) {
    return <p className="font-body text-council-ink/60">Election not found.</p>;
  }

  const { data: candidates } = await supabase
    .from("candidates")
    .select("id, category, status, round, people:person_id(first_name, last_name)")
